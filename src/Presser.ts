...
class Presser {
     ...
     * @returns {Buffer}
     */
    static decompress(buffer: BufferReader, compressedTodo: number, decompressedTodo: number): Uint8Array {
        ...
        while (pos < compressedTodo) {
            ...
            if (flag == 0xFF) {
                ...
                buffer.seek(-1);
                // read in the block size
                block_size = buffer.readLZXInt16(typeof this.buffer !== 'undefined');
                // set the frame size
                frame_size = 0x8000;

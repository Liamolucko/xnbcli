...
class Lzx {
     ...
     * @returns {Number[]}
     */
    decompress(buffer: BufferReader, frame_size: number, block_size: number): number[] {
                ...
                switch (this.block_type) {
                    ...
                    case BLOCKTYPE.UNCOMPRESSED:
                        ...
                        for (let i = 0; i < this_run; i++)
                            this.win[this.window_posn + i] = buffer.buffer[buffer.bytePosition + i];
     ...
     * @returns {Array}
     */
    readLengths(buffer: BufferReader, table: Array<any>, first: number, last: number): Array<any> {
     ...
     * @returns {Number}
     */
    readHuffSymbol(buffer: BufferReader, table: number[], length: number[], symbols: number, bits: number): number {

...
import XactSound from "./XactSound.ts";
...
class SoundBank {
  ...
  /**
     ...
     */
  static _proccess(buffer: BufferReader) {
    ...
    const cueNames = buffer.read(cueNameTableLen).toString().split("\0").slice(
      0,
      -1,
    );
    ...
    for (let i = 0; i < numComplexCues; i++) {
      ...
      if (((flags >> 2) & 1) != 0) {
        ...
        Log.debug(`Found ${bold(green(cueNames[numSimpleCues + i]))}`);
        ...
        Log.debug(
          `Found ${bold(yellow(numEntries.toString()))} cues for ${
            bold(green(cueNames[numSimpleCues + i]))
          }`,
        );
        ...
        for (let j = 0; j < numEntries; j++) {
          switch (tableType) {
            case 0: { // Wave
              ...
              Log.debug(inverse(`WaveBank Index: ${waveBankIndex}`));
              break;
            }
            case 1: {
              const soundOffset = buffer.readUInt32();
    ...
    Log.debug(`Found ${bold(red(totalCueCount.toString()))} cues!`);
  }
}

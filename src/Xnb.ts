...
import Presser from './Presser.ts';
...
class Xnb {
    ...
    convert(json: any) {
        ...
        try {
            ...
            buffer.write(encoder.encode(this.target));
            buffer.writeByte(this.formatVersion);
            // write the LZ4 mask for android compression only
            buffer.writeByte(this.hidef ? 1 : 0 | ((this.compressed && lz4Compression) ? COMPRESSED_LZ4_MASK : 0));
            ...
            if (lz4Compression) {
                ...
                buffer.dataView.setUint32(contentBuffer.length, 10);
                // write the file size into the buffer
                buffer.dataView.setUint32(XNB_COMPRESSED_PROLOGUE_SIZE + compressedSize, 6);
            ...
            // write the file size into the buffer
            buffer.dataView.setUint32(buffer.bytePosition, 6)
    ...
    _validateHeader() {
        ...
        // read the flag bits
        const flags = this.buffer.readByte();
        // get the HiDef flag
